...
const pendingTrustInvites = new Map();
...
system.runInterval(() => {
    for (const player of world.getPlayers()) {
        ...
        // Check Pending Gifts (Merged into Trust Invites logic)
        // We will store gifts in pendingTrustInvites with type 'gift'
        /*
        if (pendingGifts.has(player.id)) {
            const gift = pendingGifts.get(player.id);
            pendingGifts.delete(player.id); // Remove immediately to prevent loop, handle response
            showGiftReceivedForm(player, gift);
        }
        */

        // Check Pending Trust Invites (Now Social Invites)
        if (pendingTrustInvites.has(player.id)) {
            // player.sendMessage("Debug: Trust invite found for " + player.name);
            // We don't auto-show anymore, but we notify
            // Actually, we already notified when sent.
            // But if they just logged in?
            // Let's notify if they have invites.
            // But we don't want to spam.
            // Let's just leave it to the manual menu check.
        }
    }
}, 20);
...
world.beforeEvents.playerInteractWithEntity.subscribe(ev => {
    ...
    // If holding Catcher, let the catching logic handle it (it cancels event anyway)
    if (itemStack && itemStack.typeId === CATCHER_ID) return;
    ...
    if (ALLOWED_ENTITIES.includes(entity.typeId)) {
        ...
        if (!ownerId && tameable) {
            ownerId = tameable.tamedToPlayerId;
            if (!ownerId && tameable.owner) {
                ownerId = tameable.owner.id;
            }
        }

        // player.sendMessage(`Debug: Ride Check - Entity: ${entity.typeId}, Owner: ${ownerId}, Player: ${player.id}`);
        player.sendMessage(`Debug: Ride Check - Entity: ${entity.typeId}, Owner: ${ownerId}, Player: ${player.id}`);

        if (ownerId && ownerId !== player.id) {
            if (!isTrusted(ownerId, player.id)) {
                player.sendMessage(`Debug: Not Trusted. Owner: ${ownerId}, Player: ${player.id}`);
                // player.sendMessage(`Debug: Not Trusted. Owner: ${ownerId}, Player: ${player.id}`);
                player.onScreenDisplay.setActionBar(translateForPlayer(player, "ride_fail_not_trusted"));
                player.playSound("beacon.deactivate", { pitch: 0.8, volume: 1.0 });
                ev.cancel = true;

                // Force dismount in next tick as backup
                system.run(() => {
                    const rideable = entity.getComponent("minecraft:rideable");
                    if (rideable) {
                ...
                // player.sendMessage(`Debug: Trusted!`);
            }
        }
    }
});
...
function showTrustInvitationsForm(player) {
    const invites = pendingTrustInvites.get(player.id) || [];
...
function showTrustInviteActionForm(player, invite, index, canAccept) {
    ...
    player.sendMessage(`Debug: Invite Type: ${invite.type}, IsGift: ${isGift}`);
    ...
    const form = new MessageFormData()
        ...
        .body(bodyText)
        .button1("Deny") // Was Accept, now Deny label
        .button2(canAccept ? "Accept" : "§cAccept (Locked)"); // Was Deny, now Accept label
    ...
    form.show(player).then(res => {
        ...
        // MessageFormData: 
        // selection 1 = button 1 (Deny)
        // selection 0 = button 2 (Accept)

        // MessageFormData: 
        // button1 ("Deny") -> index 0
        // button2 ("Accept") -> index 1
        ...
        if (res.selection === 1 && canAccept) { // Button 2 (Accept)
            ...
            if (isGift) {
                ...
                if (storage.length >= MAX_STORAGE) {
                    ...
                    pendingTrustInvites.set(player.id, invites);
    ...
    form.show(player).then(res => {
        ...
        if (action === "tame") {
            if (consumeMelonSlices(player, 20)) {
                ...
                player.sendMessage(translateForPlayer(player, "tame_success"));
...
function sendGift(player, target, mob, index, type, isReturn = false) {
    ...
    const invites = pendingTrustInvites.get(target.id) || [];
    invites.push({
        type: 'gift',
        senderId: player.id,
    ...
    pendingTrustInvites.set(target.id, invites);
